...
export default function DashboardContent({ petitions, staffs }: PetitionsContentProps) {
  ...
  // Move petition counting logic to useEffect to prevent infinite re-renders
  useEffect(() => {
    // Reset counts when petitions change
    let newPetitions = 0;
    let inProgressPetitions = 0;
    let completedPetitions = 0;

    // Create a new staff petition count object
    const newStaffPetitionCountObject: Record<string, number> = {};

    petitions?.forEach((petition) => {
      if (petition.status === "New") {
        newPetitions++;
      } else if (petition.status === "In Progress") {
        inProgressPetitions++;
      } else if (petition.status === "Completed") {
        completedPetitions++;
      }
      
      // Count petitions by staff
      if (petition.assigned_to !== null) {
        newStaffPetitionCountObject[petition.assigned_to] = (newStaffPetitionCountObject[petition.assigned_to] || 0) + 1;
      }

    }
    );

    setNewCount(newPetitions);
      ...
      >
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card className="bg-slate-900/70 backdrop-blur-sm border-slate-800/50 hover:border-violet-500/30 transition-all duration-300 hover:shadow-[0_0_15px_rgba(139,92,246,0.1)] overflow-hidden">
            ...
            <CardContent className="h-80 relative z-10">
              <ResponsiveContainer width="100%" height="100%">
                <RechartsBarChart data={staffPetitionCount} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                  <XAxis dataKey="name" stroke="#94a3b8" />
                  <YAxis stroke="#94a3b8" />
                  <Tooltip
