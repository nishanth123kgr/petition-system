...
import { LayoutDashboard, Users, FileText, BarChart, Settings, LogOut, User, Menu, ChevronRight, Bell, Search, Plus, HelpCircle, Moon, Sun } from "lucide-react"
      ...
      <header className="bg-slate-900/40 backdrop-blur-xl border-b border-slate-800/60 sticky top-0 z-40 shadow-lg shadow-slate-950/30">
        <div className="container mx-auto px-4 py-3 flex items-center">
          <div className="flex items-center gap-2 mr-4">
            ...
            <Link href="/dashboard/department-admin" className="flex items-center">
          ...
          <div className={`${isSearchOpen ? 'flex flex-1' : 'hidden md:flex'} relative flex-1 max-w-md`}>
            <div className="relative w-full">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
              <Input 
                placeholder="Search petitions, staff..." 
                className="pl-9 pr-4 py-2 w-full bg-slate-800/60 border-slate-700/50 text-slate-300 focus-visible:ring-violet-500/70 focus-visible:ring-offset-slate-900"
              />
            </div>
          </div> */}
                ...
                <div className="flex flex-col">
                  {/* <Button variant="ghost" className="flex items-center justify-start gap-2 rounded-none text-slate-200 hover:text-white py-6 hover:bg-slate-800">
                    ...
                    <div className="flex flex-col items-start">
                      <span className="text-sm font-medium">New Petition</span>
                      <span className="text-xs text-slate-400">Create a new petition</span>
                    </div>
                  </Button> */}
                  <Button variant="ghost" className="flex items-center justify-start gap-2 rounded-none text-slate-200 hover:text-white py-6 hover:bg-slate-800">
                    ...
                    <div className="flex flex-col items-start">
                      ...
                      <span className="text-xs text-slate-400">Add staff member</span>
                    </div>
                  </Button>
                </div>
              </PopoverContent>
      ...
      <div className="flex-1 flex flex-col md:flex-row">
        ...
        <main className="flex-1 p-4 md:p-8 md:pt-6 md:ml-64 transition-all duration-300">
          ...
          <div className="mb-6">
            ...
            >
              <Link href="/dashboard/department-admin" className="hover:text-violet-300 transition-colors flex items-center">
                <LayoutDashboard className="h-3.5 w-3.5 mr-1.5" />
                Dashboard
              </Link>
              {pathname !== "/dashboard/department-admin" && (
                <>
...
const navItems = [
  { href: "/dashboard/department-admin", label: "Dashboard", icon: LayoutDashboard, isPro: false },
  { href: "/dashboard/department-admin/petitions", label: "Petitions", icon: FileText, isPro: false },
  { href: "/dashboard/department-admin/staff", label: "Staff", icon: Users, isPro: false },
  { href: "/dashboard/department-admin/analytics", label: "Analytics", icon: BarChart, isPro: false },
  { href: "/dashboard/department-admin/settings", label: "Settings", icon: Settings, isPro: false },
]

// Shared component for nav items
function NavItem({ item, onClick }: {
  item: { href: string; label: string; icon: any; isPro?: boolean };
  onClick?: () => void;
}) {
  const pathname = usePathname();
